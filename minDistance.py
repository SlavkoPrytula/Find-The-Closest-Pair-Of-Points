...
def closest_pair(P, Q):
    ...
    Qy = [point for point in Qn if point[0] <= P[midPoint][0]]  # exclude points from left
    Ry = [point for point in Qn if point[0] > P[midPoint][0]]  # exclude points from right

    # Recursively call
    dLeft, p1Left_x, p1Left_y, p2Left_x, p2Left_y = closest_pair(Qx, Qy)  # Left side min
    dRight, p1Right_x, p1Right_y, p2Right_x, p2Right_y = closest_pair(Rx, Ry)  # Right side min
